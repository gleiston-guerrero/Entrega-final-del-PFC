...
El resultado independiente es Auth 12/12, Usuarios 44/44, Académico 71/71 y Reservas 80/80 con ARBITER habilitado. El contrato de Reservas incluye `POST /api/v1/observabilidad/mobile/http-latency`. Reservas tiene 78 operaciones activas por defecto: las dos rutas bajo `/api/v1/internal/experimentos/arbiter` solo aparecen con `app.experimental.arbiter.enabled=true`. Por tanto, el contrato de 80 representa la **superficie habilitable**, no la superficie activa predeterminada. Una segunda prueba arranca el contexto con el flag deshabilitado y acredita 78 operaciones y ausencia de ambas rutas.
